    ...
    def test_uri(self):
        ...
        with open(temp_stpd, 'w') as stpd_file:
            Stepper(rps_schedule=["const(1,10)"],
                    ...
                    ammo_limit=-1,
                    uris=["/", "/test"],
                    headers=["[Host: ya.ru]", "[Connection: close]"], ).write(
                        stpd_file)
        res = open(temp_stpd, 'r').read()
        self.assertNotEquals("", res)
    ...
    def test_access_log(self):
        ...
        with open(temp_stpd, 'w') as stpd_file:
            Stepper(rps_schedule=[],
                    ...
                    ammo_type='access',
                    headers=[],
                    ammo_file="data/access1.log", ).write(stpd_file)
        res = open(temp_stpd, 'r').read()
        self.assertNotEquals("", res)
        ...
        with open(temp_stpd, 'w') as stpd_file:
            with self.assertRaises(AmmoFileError):
                Stepper(rps_schedule=[],
                        ...
                        ammo_limit=10,
                        headers=[],
                        ammo_file="data/empty.uri", ).write(stpd_file)
    ...
    def test_chosen_cases(self):
        ...
        with open(temp_stpd, 'w') as stpd_file:
            Stepper(rps_schedule=["const(1, 10)"],
                    ...
                    chosen_cases="_one", ).write(stpd_file)
        res = open(temp_stpd, 'r').read()
        self.assertNotEquals("", res)
