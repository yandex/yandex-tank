    ...
    def main(self):
        ...
        if tank_type == 1:
            if self.uris:
                ...
                ammo_file = make_load_ammo(self.uris, self.headers, self.header_http)
                ammo_type = "uri"
        ...
        if tank_type == 1:
            if not self.autocases:
                ...
                if ammo_type == 'request' and detect_case_file(ammo_file) == True:
                    ...
                    if self.autocases:
                        (cases_done, cases_output) = make_autocases_top(l1, l2, l3, ammo_count, cases_tree)
        ...
        ### Case 0. Neither 'load' nor 'loop' present
        ...
        if loop == 0 and ammo_count < load_count:
            case = 1
            raise RuntimeError("Not enough ammo (%s) in '%s' for load scheme (%s)" % (ammo_count, ammo_file, load_count))
            ...
            if case == 2:
                ...
                for l in range(1, loop + 1):
                    for line in input_ammo:
                        ...
                        if m:
                            real_case = get_prepared_case(line.rstrip(), cases_done, pattern)
                            chunk = chunk_by_uri(line.rstrip(), http, 1000, real_case, header_common, header_config)
                            stepped_ammo.write(chunk)
                            cur_progress += 1
                            pbar.update(cur_progress)
                            #sleep(1)
                    ...
                    while looping:
                        for line in input_ammo:
                            ...
                            if m:
                                ...
                                real_case = get_prepared_case(line.rstrip(), cases_done, pattern)
                                chunk = chunk_by_uri(line.rstrip(), http, base_time + time, real_case, header_common, header_config)
                                stepped_ammo.write(chunk)
                                cur_progress += 1
                                step_ammo_num += 1
                                pbar.update(cur_progress)
