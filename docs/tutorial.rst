...
To specify external ammo file use `ammofile` option. Use `ammo_type` option to explicitly
specify ammo format (if ammo type is uri-style or request-style, tank will try to guess it).
