...
class DistributedPluginTestCase(TankTestCase):
    ...
    def test_run(self):
        ...
        for mock in self.foo.api_clients:
            ...
            mock.get_data.append({"status": TankAPIClient.STATUS_FINISHING, "exclusive": 1})
            mock.get_data.append({"status": TankAPIClient.STATUS_FINISHING, "exclusive": 1})
            mock.get_data.append({"status": TankAPIClient.STATUS_FINISHED, "exclusive": 1})
            mock.get_data.append({"status": TankAPIClient.STATUS_FINISHED, "artifacts": ["tank.log", "test.log", "phout.txt"]})
