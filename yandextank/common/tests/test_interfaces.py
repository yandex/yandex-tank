...
class TestStatus(object):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [(['plugin1', 'key1'], 'foo'), (['plugin1', 'key2'], 42), (['plugin2', 'key1'], 'bar')],
                {'plugin1': {'key1': 'foo', 'key2': 42}, 'plugin2': {'key1': 'bar'}},
            ),
        ],
    )
...
@pytest.mark.parametrize(
    'codes_mask, codes_dict, expected_matched_cnt',
    [
...
def test_match(codes_mask, codes_dict, expected_matched_cnt):
    assert (
        AbstractCriterion.count_matched_codes(codes_regex=re.compile(codes_mask), codes_dict=codes_dict)
        == expected_matched_cnt
    )
