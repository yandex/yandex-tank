...
class UriValidator(FileFormatValidator):
    ...
    def is_suitable(self, features: Features) -> Decision:
        ...
        sline = features.first_line().split()
        if len(sline) > 0 and sline[0].startswith(b'/'):
            return Decision({AmmoType.URI})
        return Decision(set())
    ...
    def _check_header(self, messages: Messages, start_offset: int, header: bytes):
        if b':' not in header:
            messages.error(self._msg('Header line does not contain ":"', file_offset=start_offset))
        ...
        for line in stream:
            ...
            if line.startswith(b'['):
                if not line.endswith(b']'):
                    messages.error(self._msg('Header line does not end with "]"', file_offset=start_offset))
                self._check_header(messages, start_offset, line.strip(b'\r\n[]\t '))
            else:
                fields = line.split()
                if len(fields) >= 3:
                    messages.warning(self._msg('Too many tags. Only one tag is allowed', file_offset=start_offset))
                success += 1

            start_offset = stream.tell()

        messages.info(self._msg(f'{count} non empty lines read ({success} uris)'))
        if not success:
