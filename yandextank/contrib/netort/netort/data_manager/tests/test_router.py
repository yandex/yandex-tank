...
class TestAggregatorBuffer(object):
    ...
    def test_buffer_no_last_piece(self):
        ...
        assert len(self.df1) + len(self.df2) > len(res1) + len(res2)
        assert len(self.df1) + len(self.df2) == len(res1) + len(res2) + len(
            self.metrics_router._MetricsRouter__aggregator_buffer.get('metric1', [])
        )
    ...
    def test_buffer_multiple_metrics(self):
        ...
        assert len(self.df1) + len(self.df2) == len(res11) + len(res12) + len(
            self.metrics_router._MetricsRouter__aggregator_buffer.get('metric1', [])
        )
