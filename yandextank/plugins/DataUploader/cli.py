...
def get_logger():
    ...
    verbose_handler = logging.FileHandler(datetime.now().strftime("post_loader_%Y-%m-%d_%H-%M-%S.log"), 'w')
...
def post_loader():
    CONFIG_SCHEMA = load_yaml_schema(
        pkg_resources.resource_filename('yandextank.plugins.DataUploader', 'config/postloader_schema.yaml')
    )
    ...
    parser.add_argument(
        '-a', '--api_address', help='service to upload test results to, e.g. https://overload.yandex.net'
    )
    ...
    parser.add_argument('test_dir', help='Directory containing test artifacts')
    ...
    user_agent = ' '.join(('Uploader/{}'.format(DataUploader.VERSION), TankCore.get_user_agent()))
    api_client = client(
    ...
    logger.info('LP job created: {}'.format(urljoin(api_client.base_url, str(lp_job.number))))
