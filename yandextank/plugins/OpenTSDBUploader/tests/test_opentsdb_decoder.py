...
class TestDecoder(object):
    def test_metrics_cast(self):
        ...
        timestamp = 123456789
        host = '127.0.0.1'
        data = [{'data': {host: {'comment': comment, 'metrics': raw_metrics}}, 'timestamp': timestamp}]
        expected_metrics = {
            'metric1': -123.0,
            'metric2': -123.456,
            'metric3': 123.0,
        ...
        for r_point in result_points:
            ...
            if r_point['tags']['field'] not in expected_metrics:
                assert False
            if not isinstance(r_point['value'], type(expected_metrics[r_point['tags']['field']])):
                assert False
            if not r_point['value'] == expected_metrics[r_point['tags']['field']]:
                assert False
