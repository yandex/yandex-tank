...
class SSHClient(object):
        ...
        while not done:
            ...
            if self.session.finished():
                logger.debug('Session ended with status %s with %s',
                    self.session.exit_status(), self.host)
    ...
    def _kill_agent(self):
        if self.agent_remote_folder:
            forced_stop = ('main_pid=$(pgrep -f "[p]ython.*{folder}");'
                   'descendent_pids(){{ if [ "x$1" != "x" ]; then '
                   'pids=$(pgrep -P $1); echo $pids;'
                   'for p in $pids; do descendent_pids $p; done; fi }};'
                   'tlgrf_pid=$(pgrep -f "[t]elegraf.*{folder}");'
                   'all_pids=$(descendent_pids ${{main_pid}});'
                   'if [ "x${{main_pid}}${{all_pids}}${{tlgrf_pid}}" != "x" ]; then'
                   ' kill -9 ${{main_pid}} ${{all_pids}} ${{tlgrf_pid}}; fi;')

            cmd = forced_stop.format(folder=self.agent_remote_folder)
            out, errors, err_code = self.ssh.execute(cmd)
            if errors:
                logger.error(
