...
class TestConfigManager(object):
    ...
    def test_xml_old_parse(self):
        ...
        manager = ConfigManager()
        configs = manager.getconfig('yandextank/plugins/Telegraf/tests/old_mon.xml', 'sometargethint')
        assert (
    ...
    def test_xml_telegraf_parse(self):
        ...
        manager = ConfigManager()
        configs = manager.getconfig('yandextank/plugins/Telegraf/tests/telegraf_mon.xml', 'sometargethint')
        assert (
            configs[0]['host'] == 'somehost.yandex.tld'
            and
            configs[0]['host_config']['CPU']['name'] == '[inputs.cpu]'
    ...
    def test_target_hint(self):
        ...
        manager = ConfigManager()
        configs = manager.getconfig('yandextank/plugins/Telegraf/tests/target_hint.xml', 'somehost.yandex.tld')
...
class TestAgentConfig(object):
    def test_create_startup_configs(self):
        ...
        manager = ConfigManager()
        telegraf_configs = manager.getconfig('yandextank/plugins/Telegraf/tests/telegraf_mon.xml', 'sometargethint')
    ...
    def test_create_collector_configs(self):
        ...
        assert (
            ...
            and
            cfg_parser.get('agent', 'interval') == "'1s'"
            and
            cfg_parser.has_section('[outputs.file')
            and
            cfg_parser.get('[outputs.file', 'files') == "['{rmt}/monitoring.rawdata']".format(rmt=remote_workdir)
        )
