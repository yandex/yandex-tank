...
def get_data(cnt_by_code):
    return {
        "overall": {"net_code": {"count": cnt_by_code},
                    "interval_real": {"len": sum(cnt_by_code.values())}},
        "ts": int(time.time())
    }
...
@pytest.mark.parametrize('not_matched_codes',
                         ['1%', '50%', '100%',
                          '1', '50', '100'])
def test_neg_net_code_notify_is_false_when_no_other_codes(code, not_matched_codes):
    criterion = TotalNegativeNetCodesCriterion(autostop=None,
                                               param_str=f"{code}, {not_matched_codes}, 10s")
...
def test_neg_net_code_notify_is_true_when_only_other_codes(code, not_matched_codes):
    criterion = TotalNegativeNetCodesCriterion(autostop=None,
                                               param_str=f"{code}, {not_matched_codes}, {WINDOW_SIZE}s")
